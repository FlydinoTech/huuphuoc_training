@extends('template.booktour.master')
@section('main_content')
    <!--Shop Banner Section-->
    <section class="shop-banner-section" style="background-image:url(img/tour/background/8.jpg);">
        <div class="auto-container">
            
            <!-- Content Box -->
            <div class="content-box">
                <div class="box-inner">
                    <h2>Modern Furniture</h2>
                    <div class="text">Leverage agile frameworks to provide a robust synopsis for high level overviews. Iterative approaches to corporate strategy foster collaborative.</div>
                    <a href="product-detail.html" class="theme-btn btn-style-one"><span class="txt">purchase now</span></a>
                </div>
            </div>
            
        </div>
    </section>
    <!--End Shop Banner Section-->
    
    <!--Shop Features Section-->
    <section class="shop-features-section">
        <div class="auto-container">
            <div class="row clearfix">
                <!--Feature Block-->
                <div class="shop-feature-block col-lg-4 col-md-6 col-sm-12">
                    <div class="inner-box">
                        <div class="icon-box">
                            <span class="fa fa-truck"></span>
                        </div>
                        <h3><a href="#">Free Shipping</a></h3>
                        <div class="text">Duis aute irure dolor in reprehend erit in voluptate velit esse cillu.</div>
                    </div>
                </div>
                
                <!--Feature Block-->
                <div class="shop-feature-block col-lg-4 col-md-6 col-sm-12">
                    <div class="inner-box">
                        <div class="icon-box">
                            <span class="fa fa-tag"></span>
                        </div>
                        <h3><a href="#">Sale 15% Off</a></h3>
                        <div class="text">Duis aute irure dolor in reprehend erit in voluptate velit esse cillu.</div>
                    </div>
                </div>
                
                <!--Feature Block-->
                <div class="shop-feature-block col-lg-4 col-md-6 col-sm-12">
                    <div class="inner-box">
                        <div class="icon-box">
                            <span class="fa fa-adjust"></span>
                        </div>
                        <h3><a href="#">New Arriaval</a></h3>
                        <div class="text">Duis aute irure dolor in reprehend erit in voluptate velit esse cillu.</div>
                    </div>
                </div>
                
            </div>
        </div>
    </section>
    <!--End Shop Features Section-->
    
    <!--Shop Section-->
    <section class="shop-section">
        <div class="auto-container">
            <!--Sec Title-->
            <div class="title-box">
                <h2>Featured Products</h2>
            </div>
            
            <div class="row clearfix">
                
                <!--Shop Item-->
                <div class="shop-item-two col-lg-3 col-md-3 col-sm-12">
                    <div class="inner-box">
                        <div class="image">
                            <a href="product-detail.html"><img src="img/tour/resource/products/5.jpg" alt="" /></a>
                        </div>
                        <div class="lower-content">
                            <h3><a href="product-detail.html">COLD CREWNECK</a></h3>
                            <div class="price">$39.99</div>
                            <a href="product-detail.html" class="theme-btn cart-btn">Add to cart</a>
                        </div>
                    </div>
                </div>
                
                <!--Shop Item-->
                <div class="shop-item-two col-lg-3 col-md-3 col-sm-12">
                    <div class="inner-box">
                        <div class="image">
                            <a href="product-detail.html"><img src="img/tour/resource/products/5.jpg" alt="" /></a>
                        </div>
                        <div class="lower-content">
                            <h3><a href="product-detail.html">COLD CREWNECK</a></h3>
                            <div class="price">$39.99</div>
                            <a href="product-detail.html" class="theme-btn cart-btn">Add to cart</a>
                        </div>
                    </div>
                </div>
                
                <!--Shop Item-->
                <div class="shop-item-two col-lg-3 col-md-3 col-sm-12">
                    <div class="inner-box">
                        <div class="image">
                            <a href="product-detail.html"><img src="img/tour/resource/products/5.jpg" alt="" /></a>
                        </div>
                        <div class="lower-content">
                            <h3><a href="product-detail.html">COLD CREWNECK</a></h3>
                            <div class="price">$39.99</div>
                            <a href="product-detail.html" class="theme-btn cart-btn">Add to cart</a>
                        </div>
                    </div>
                </div>
                
                <!--Shop Item-->
                <div class="shop-item-two col-lg-3 col-md-3 col-sm-12">
                    <div class="inner-box">
                        <div class="image">
                            <a href="product-detail.html"><img src="img/tour/resource/products/5.jpg" alt="" /></a>
                        </div>
                        <div class="lower-content">
                            <h3><a href="product-detail.html">COLD CREWNECK</a></h3>
                            <div class="price">$39.99</div>
                            <a href="product-detail.html" class="theme-btn cart-btn">Add to cart</a>
                        </div>
                    </div>
                </div>

                <!--Shop Item-->
                <div class="shop-item-two col-lg-3 col-md-3 col-sm-12">
                    <div class="inner-box">
                        <div class="image">
                            <a href="product-detail.html"><img src="img/tour/resource/products/5.jpg" alt="" /></a>
                        </div>
                        <div class="lower-content">
                            <h3><a href="product-detail.html">COLD CREWNECK</a></h3>
                            <div class="price">$39.99</div>
                            <a href="product-detail.html" class="theme-btn cart-btn">Add to cart</a>
                        </div>
                    </div>
                </div>
                
                <!--Shop Item-->
                <div class="shop-item-two col-lg-3 col-md-3 col-sm-12">
                    <div class="inner-box">
                        <div class="image">
                            <a href="product-detail.html"><img src="img/tour/resource/products/5.jpg" alt="" /></a>
                        </div>
                        <div class="lower-content">
                            <h3><a href="product-detail.html">COLD CREWNECK</a></h3>
                            <div class="price">$39.99</div>
                            <a href="product-detail.html" class="theme-btn cart-btn">Add to cart</a>
                        </div>
                    </div>
                </div>
                
                <!--Shop Item-->
                <div class="shop-item-two col-lg-3 col-md-3 col-sm-12">
                    <div class="inner-box">
                        <div class="image">
                            <a href="product-detail.html"><img src="img/tour/resource/products/5.jpg" alt="" /></a>
                        </div>
                        <div class="lower-content">
                            <h3><a href="product-detail.html">COLD CREWNECK</a></h3>
                            <div class="price">$39.99</div>
                            <a href="product-detail.html" class="theme-btn cart-btn">Add to cart</a>
                        </div>
                    </div>
                </div>
                
                <!--Shop Item-->
                <div class="shop-item-two col-lg-3 col-md-3 col-sm-12">
                    <div class="inner-box">
                        <div class="image">
                            <a href="product-detail.html"><img src="img/tour/resource/products/5.jpg" alt="" /></a>
                        </div>
                        <div class="lower-content">
                            <h3><a href="product-detail.html">COLD CREWNECK</a></h3>
                            <div class="price">$39.99</div>
                            <a href="product-detail.html" class="theme-btn cart-btn">Add to cart</a>
                        </div>
                    </div>
                </div>	
            </div>
            <div class="shop-pagination">
                <ul class="clearfix">
                    <li class="prev"><a href="#"><i class="fa fa-angle-left"></i></a></li>
                    <li><a href="#">1</a></li>
                    <li class="active"><a href="#">2</a></li>
                    <li><a href="#">3</a></li>
                    <li class="next"><a href="#"><i class="fa fa-angle-right"></i></a></li>
                </ul>
            </div>

        </div>
    </section>
    
    <!--Products Section-->
    <section class="products-section">
        <div class="auto-container">
            <div class="row clearfix">
                <!--Product Column-->
                <div class="product-column col-lg-6 col-md-12 col-sm-12">
                    <div class="row clearfix">
                        <!--Shop Item Two-->
                        <div class="shop-item-two col-lg-6 col-md-6 col-sm-12">
                            <div class="inner-box">
                                <div class="image">
                                    <a href="product-detail.html"><img src="img/tour/resource/products/5.jpg" alt="" /></a>
                                </div>
                                <div class="lower-content">
                                    <h3><a href="product-detail.html">COLD CREWNECK</a></h3>
                                    <div class="price">$39.99</div>
                                    <a href="product-detail.html" class="theme-btn cart-btn">Add to cart</a>
                                </div>
                            </div>
                        </div>
                        
                        <!--Shop Item Two-->
                        <div class="shop-item-two col-lg-6 col-md-6 col-sm-12">
                            <div class="inner-box">
                                <div class="image">
                                    <a href="product-detail.html"><img src="img/tour/resource/products/6.jpg" alt="" /></a>
                                </div>
                                <div class="lower-content">
                                    <h3><a href="product-detail.html">MULTI-WAY ULTRA</a></h3>
                                    <div class="price">$39.99</div>
                                    <a href="product-detail.html" class="theme-btn cart-btn">Add to cart</a>
                                </div>
                            </div>
                        </div>
                        
                    </div>
                </div>
                
                <!--Content Column-->
                <div class="content-column col-lg-6 col-md-12 col-sm-12">
                    <div class="inner-column">
                        <h2>Best products <span>for home</span></h2>
                        <div class="text">Nemo enim ipsam voluptatem quia voluptas sit aspernatur aut odit aut fugit, sed quia consequuntur magni dolores eos qui ratione voluptatem sequi nesciunt. </div>
                        <a href="#" class="theme-btn btn-style-one"><span class="txt">View all products</span></a>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!--End Products Section-->
@stop